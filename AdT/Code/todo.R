...
library(lubridate)
adt$click_time <- ymd_hms(adt$click_time)
...
adt[, clicker_app_Next := tdiff(click_time), by = list(ip, device, os, app)]
adt[, clicker_ch_Next := tdiff(click_time), by = list(ip, device, os, app, channel)]
