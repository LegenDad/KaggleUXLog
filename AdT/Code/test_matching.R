...
test <- fread("../input/test.csv", colClasses = list(numeric=2:6))
testsup <- fread("../input/test_supplement.csv", colClasses = list(numeric=2:6))

# Add dup_order column to specify the order of duplicate rows
test[, dup_order := seq(.N), by = .(ip, app, device, os, channel, click_time)]
testsup[, dup_order := seq(.N), by = .(ip, app, device, os, channel, click_time)]
...
fwrite(result, "test_click_id_relation.csv")
