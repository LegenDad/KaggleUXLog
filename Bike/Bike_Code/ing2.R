...
cas_lgtr <- lgb.Dataset(data = data.matrix(tr[index,]), 
                        label = cas[index])

cas_lgval <- lgb.Dataset(data = data.matrix(tr[-index,]), 
                         label = cas[-index])
...
te <- te %>% cbind(casual = cas_lgpred)

# reg_xgtr <- xgb.DMatrix(data = data.matrix(tr[index,]), 
#                         label = reg[index])
# reg_xgval <- xgb.DMatrix(data = data.matrix(tr[-index,]), 
#                          label = reg[-index])
# reg_xgb <- xgb.train(p, reg_xgtr, p$nrounds, list(val = reg_xgval), 
#                      print_every_n = 10, early_stopping_rounds = 200)

reg_lgtr <- lgb.Dataset(data = data.matrix(tr[index,]), 
                        label = reg[index])
reg_lgval <- lgb.Dataset(data = data.matrix(tr[-index,]), 
                         label = reg[-index])
...
# reg_xgte <- xgb.DMatrix(data = data.matrix(te))
# reg_xgpred <- predict(reg_xgb, reg_xgte)

reg_lgte <- data.matrix(te)
...
te <- te %>% cbind(registered = reg_lgpred)
